...
const CategorySection: React.FC<CategorySectionProps> = ({ category, items, selectedItems, onToggle, onUpdateQuantity }) => {
    const [isExpanded, setIsExpanded] = useState(true);
    const Icon = iconMap[category.iconSlug] || Volume2;

    const selectedCount = items.filter((item) => selectedItems.get(item.id)?.isSelected).length;

    return (
        <div className="rounded-2xl border border-border bg-card overflow-hidden">
            <button onClick={() => setIsExpanded(!isExpanded)} className="flex w-full items-center justify-between gap-4 p-5 text-left transition-colors hover:bg-muted/30">
            ...
            <div className={cn("grid transition-all duration-300", isExpanded ? "grid-rows-[1fr]" : "grid-rows-[0fr]")}>
                <div className="overflow-hidden">
                    <div className="flex flex-col gap-3 p-5 pt-0">
                        {items.map((item) => (
                            <ItemRow key={item.id} item={item} selectedItem={selectedItems.get(item.id)} onToggle={onToggle} onUpdateQuantity={onUpdateQuantity} />
                        ))}
                    </div>
                </div>
            </div>
